...
// This API is used EXCLUSIVELY by the Admin Manage Riders performance modal.
// We strictly require the Admin context here.
if ($contextMode !== 'admin' || !$riderId) {
...
if (!$riderId) {
    echo json_encode(['success' => false, 'error' => 'Rider ID missing']);
    exit;
}
...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        FROM orders o
        WHERE o.rider_id = :rider_id AND (
            (o.grabbed_at >= :gs AND o.grabbed_at <= :ge) OR 
            (o.delivered_at >= :ds AND o.delivered_at <= :de)
        )
    ");
    $stmt->execute([
        ':s1' => $startTime,
    ...
    $cStmt = $pdo->prepare("
        ...
        WHERE rider_id = ? AND closing_date >= ? AND closing_date <= ?
    ");
    // Actually monthly tips might be better from orders tied to window
    $cStmt->execute([$riderId, $monthFirstDay, $monthLastDay]);
    $cMonthly = $cStmt->fetch(PDO::FETCH_ASSOC);

    // Get Monthly Tips from orders

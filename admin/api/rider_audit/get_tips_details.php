...
try {
    $stmt = $pdo->prepare("
        ...
        FROM (
            SELECT id, status, delivered_at, rider_tip, customer_name, user_id
            FROM orders
            WHERE rider_id = ? AND status = 'completed' AND rider_tip > 0 AND (delivered_at >= ? AND delivered_at <= ?)
    ...
    ob_start();
    if (empty($orders)) {
        echo '<div style="padding:40px;text-align:center;color:#64748b;">No tips recorded today</div>';
    } else {
        echo '<div style="padding:10px;">';
        foreach ($orders as $order) {
            echo '<div style="background:white;border:1.5px solid #f1f5f9;border-radius:20px;padding:16px;margin-bottom:12px;">';
            echo '<div style="display:flex;justify-content:space-between;"><div><div style="font-size:0.7rem;font-weight:800;color:#6366f1;">#ORD-'.str_pad($order['id'],5,'0',STR_PAD_LEFT).'</div><h4 style="margin:0;">'.htmlspecialchars($order['display_name']).'</h4></div><div>Rs. '.number_format($order['rider_tip']).'</div></div>';

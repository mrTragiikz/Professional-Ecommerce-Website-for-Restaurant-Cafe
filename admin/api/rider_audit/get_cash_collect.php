...
try {
    $stmt = $pdo->prepare("
        ...
        FROM (
            SELECT id, status, delivered_at, paid_amount_cash, paid_amount_online, customer_name, user_id, payment_status
            FROM orders
            WHERE rider_id = ? AND status = 'completed' AND LOWER(payment_status) = 'paid' AND paid_amount_cash > 0 AND (delivered_at >= ? AND delivered_at <= ?)
    ...
    ob_start();
    if (empty($orders)) {
        echo '<div style="padding:40px;text-align:center;color:#64748b;">No cash collections found</div>';
    } else {
        echo '<div style="padding:10px;">';
        foreach ($orders as $order) {
            echo '<div style="background:white;border:1.5px solid #f1f5f9;border-radius:20px;padding:16px;margin-bottom:12px;">';
            echo '<div style="display:flex;justify-content:space-between;"><div><div style="font-size:0.7rem;font-weight:800;color:#ef4444;">#ORD-'.str_pad($order['id'],5,'0',STR_PAD_LEFT).'</div><h4 style="margin:0;">'.htmlspecialchars($order['display_name']).'</h4></div><div>Rs. '.number_format($order['paid_amount_cash']).'</div></div>';
